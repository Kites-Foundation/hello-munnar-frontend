...
let RouteBanner = (props) => {
    return <div 
            onClick={
                ()=>{
                    props.setSelectedRoute(props.RouteColor)
                    props.setSelectedIndex(props.findIndexFromData(props.RouteColor))
                }
            }
        >
        <div className="flex-shrink-0 w-72 h-40 mr-2 bg-green-500 overflow-hidden relative">
            ...
            <p className="absolute bottom-0 left-0 text-white z-20 pb-3 pl-2 text-xl font-bold">{props.RouteName}</p>
